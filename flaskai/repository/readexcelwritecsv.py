...
class ReadExcelWriteCsv:
    ...
    def __write_csv_extra_data(self, from_excel):
        l_data = 'D:/Artificial Intelligence Study/flaskai/excel_data/extra_data/' + from_excel
        x1 = pd.ExcelFile(l_data)
        df = x1.parse('Sheet1')
        to_csv = re.sub('Extra Data ', '', str(from_excel))
        to_csv2 = re.sub('.xlsx', '', to_csv)
        with io.open('D:/Artificial Intelligence Study/flaskai/csv_data/csv_extra_data/' + to_csv2, 'w', encoding="utf-8") as f:
            writer = csv.writer(f)
            for index, row in df.iterrows():
                writer.writerow(row)

    def __write_csv_info(self, from_excel, to_csv):
        liverpool_data = 'D:/Artificial Intelligence Study/flaskai/excel_data/' + from_excel
        xl = pd.ExcelFile(liverpool_data)
        df1 = xl.parse('Sheet1')

        with io.open('D:/Artificial Intelligence Study/flaskai/csv_data/' + to_csv, 'w', encoding="utf-8") as f:
            writer = csv.writer(f)
    ...
    def __eliminate_white_spaces_extra(self, in_file, out_file):
        with open('D:/Artificial Intelligence Study/flaskai/csv_data/csv_extra_data/' + in_file, 'r', encoding='utf-8') as inFile, \
                open('D:/Artificial Intelligence Study/flaskai/csv_data/csv_extra_data/' + out_file + '.txt', 'w', encoding='utf-8') as outFile:
            for line in inFile:
                if line.strip():
                    outFile.write(line)
    ...
    def __eliminate_white_spaces(self, in_file, out_file):
        with open('D:/Artificial Intelligence Study/flaskai/csv_data/' + in_file, 'r', encoding='utf-8') as inFile, \
                open('D:/Artificial Intelligence Study/flaskai/csv_data/' + out_file + '.txt', 'w', encoding='utf-8') as outFile:
            for line in inFile:
                if line.strip():
                    outFile.write(line)
